...
import processing.opengl.PGraphics2D;
  ...
  public void liquidSetup() {
    surface.setLocation(viewport_x, viewport_y);
    
    pg_particles = (PGraphics2D) createGraphics(width/aec.getScaleX(), height/aec.getScaleY(), P2D);
    pg_particles.smooth(0);
    
    pg_particles2 = (PGraphics2D) createGraphics(width/aec.getScaleX(), height/aec.getScaleY(), P2D);
    ...
    particlesystem = new ParticleSystem(this, width/aec.getScaleX(), height/aec.getScaleY());
    
    // set some parameters
    particlesystem.PARTICLE_COUNT              = 1500;
    particlesystem.PARTICLE_SCREEN_FILL_FACTOR = 1.0f;
    ...
    param_physics.bounds  = new float[]{width/2/aec.getScaleX()- 6.15, 0, width - width/2/aec.getScaleX(), height/aec.getScaleY()/1.8};
    // param_physics.bounds  = new float[]{width/2/aec.getScaleX() + 10, 0, width - width/2/aec.getScaleX(), height/aec.getScaleY()/1.8};
  ...
  if (finalCount > totalCount1234) {
    if (400 > megaCount && megaCount >= 0) {
      renderRecords = true;
    } else {
      renderRecords = false;
    }

    if ( megaCount >= 400 && megaCount < 700) {
      renderButterfly = true;
    } else {
      renderButterfly = false;
    }

    if (megaCount >= 700 && megaCount < 900) {
      renderTree = true;
    } else {
      renderTree = false;
    }

    if (megaCount >= 900 && megaCount < 1200) {
      renderBoom = true;
    } else {
      renderBoom = false;
    }
    
    if (megaCount >= 1200 && megaCount < 1500) {
      renderEyes = true;
    } else if (megaCount >= 1500){
      renderEyes = false;
      realEnd = true;
    } else {
    renderEyes = false;
    ...
    if (renderRecords) {
      background(rRecord, gRecord, bRecord);
      record();
    }

    if (!renderRecords && renderButterfly) {
      if (rRecord != 0) {
        rRecord = rRecord - 20;
      }
    ...
    if (realEnd) {
      int temp = megaCount;
      totalCount1234 = totalCount1234 + temp;
      megaCount = 0;
      reset();
    }
  ...
  megaCount++;
  System.out.println("mega count: " + megaCount);
    ...
    // info
    String txt_fps = String.format(getClass().getName()+ "   [size %d/%d]   [frame %d]   [fps %6.2f]", width/aec.getScaleX(), height/aec.getScaleY(), frameCount, frameRate);
    surface.setTitle(txt_fps);
    
     if ( beat.isOnset() ) {
    particlesystem.setFillFactor(2.5);
  }
  else {
    particlesystem.setFillFactor(1.7);
  }
}
  ...
  public void createGUI(){
    ...
    Group group_particles = cp5.addGroup("Particles");
    {
      ...
      /*
      cp5.addSlider("BOUNDARIES").setGroup(group_particles).setSize(sx, sy).setPosition(px, py+=oy)
          .setRange(0 , height/aec.getScaleY()).setValue(param_physics.bounds [3]).plugTo(param_physics.bounds [3]);
      */
    }
